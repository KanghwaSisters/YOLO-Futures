...
class MarketEnvironment:
    ...
    def is_maturity_date(self):
        flag = True if (self.current_timestep.date() in self.maturity_list) else False
        self.info = 'maturity_date' if flag is True else ''
        return flag

    
    def is_bankrupt(self):
        flag = True if (self.account.available_balance <= 0) else False
    ...
    def is_near_margin_call(self):
        # 현재 마진콜(7%) 조건
        flag = True if (self.account.available_balance <= self.account.maintenance_margin) else False
    ...
    def is_not_sufficient(self):
        flag = True if (self.account.available_balance <= self.previous_price * self.account.initial_margin_rate) else False
        self.info = 'not_sufficient' if flag is True else ''
        return flag
